...
Gdli is a Node.js client that runs in your terminal. Paired with the gdli addon inside Godot — one instance in the editor (an @tool EditorPlugin) and one in the running game (an autoload), on separate ports.
...
- **module** — your namespace (e.g. `"mygame"`). Used for grouping and `gdli config --disable mygame`.
- **name** — the verb's CLI path *and* its wire id (e.g. `"mygame spawn"`). Multi-word is fine (the
  client greedily matches the longest name, up to 4 tokens). Namespace it under your module to avoid
  colliding with the built-ins.
- **handler** — a `Callable`, `func(params: Dictionary) -> Variant`.
- **meta** — `{help, target, async?, args}`:
  - **help** — one line.
  - **target** — routing policy: `"auto"` (game if running, else the editor scene), `"game"`, or
    `"editor"`. The client picks the process from this.
  - **async** — set `true` if your handler `await`s (deferred ops); dispatch will await it.
  - **args** — `[{name, type, required, default, help}]`. A `name` without dashes is **positional**;
    with dashes it's a **flag**. `type` ∈ `string|int|float|bool|json`. The **param key your handler
    reads = the name minus leading dashes** (`--root` → `params.root`, positional `path` → `params.path`).
...
- **Decode typed inputs** with `server.from_json(...)` — it accepts the tagged form
  `{"__t":"Vector2","v":[1,2]}` *or* a Godot expression string (`"Vector2(1,2)"`). Plain
  strings/ints/bools come through as-is.
- **Return** any JSON-able value as `data` — strings/numbers/bools/arrays/dicts pass through. Engine
  types (Vector2, Color, Node, …) don't *need* encoding: `JSON.stringify` auto-converts them to a lossy
  string (`Vector2(1,2)` → `"(1.0, 2.0)"`), fine if you're just printing. Wrap with `server.to_json(...)`
  only when you want the **structured, machine-readable** form (`{"__t":"Vector2","v":[1,2]}`) —
...
| `server.to_json(v)` / `from_json(v)` | the Variant ↔ JSON codec |
...
| `server.rel_path(node)` | node → scene-relative path string |
...
| `server.mint_refs(nodes)` | mint `@eN` refs for those nodes (returns `["@e1", …]`) |
| `server.screen_pos(node\|@ref)` | a node/ref → screen position (Control center / Node2D origin, else null) |
| `server.call_gdli_string(cmd)` | run any verb by its CLI string in-process, raw result (the `gdli("…")` bridge in eval) |
| `server.err(code, msg)` | return an error |

(`--diff` / `--mark` are handled by the dispatcher around *every* verb, so plugins never touch them —
your handler just mutates the scene and the core snapshots before/after. There's no per-verb baseline API.)
...
- **Mint** — `server.mint_refs(nodes)` assigns `@e1, @e2, …` to a list of nodes and returns the labels.
...
- **Consume** — `server.resolve_node("@e2")` returns the node (it accepts a path *or* a ref everywhere),
  and `server.screen_pos("@e2")` returns its screen position. Every input verb (`click`, `drag`,
  `hover`, `scroll`) accepts a ref via `--ref`/`--ref2` in place of `x y` coordinates, by routing through
...
- **Enable/disable** — your module toggles by name: `gdli config --disable mygame` (persists, applies
  live; the server mtime-watches `config.json`). Disabled modules vanish from `verbs` and dispatch.
- **Two instances** — a `game`-target verb runs in the game process against the game server; an
  `editor`-target verb runs in the editor. `auto`-target verbs run in both, prefering game over editor.
   Each process has its own plugin instance; you never reach cross processes — the client routes per verb.
